    ...
    emitter.on('assertion', (err, args) => {
      ...
      for (let i = 0; i < strings.length; i++) {
        ...
        if (matches) {
          ...
          if (err) {
            const headerMsg = `Test failed: ${args.item.name}`;
            const assertMsg = `Assert: ${args.assertion}`;
            const errorMsg = `Error: ${err.message}`;
            const urlMsg = `${requestObj.method} ${requestObj.url} [${responseObj.code} ${responseObj.status}, ${responseObj.responseTime / 1000}s]`;
            const respMsg = `Response body: \n${responseObj.stream.toString()}`;
            const errMsg = `${headerMsg}\n${assertMsg}\n${errorMsg}\n${urlMsg}\n${respMsg}}`;
            lastResult.comment = errMsg;
          }else{
            const urlMsg = `${requestObj.method} ${requestObj.url} [${responseObj.code} ${responseObj.status}, ${responseObj.responseTime / 1000}s]`;
            lastResult.comment = urlMsg;
          }
